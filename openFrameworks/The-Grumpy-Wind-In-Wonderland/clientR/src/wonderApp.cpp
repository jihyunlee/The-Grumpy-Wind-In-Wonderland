...
void wonderApp::update(){
	...
	while( receiver.hasWaitingMessages() )
	{
		...
		if ( m.getAddress() == "/movie/position" )
		{
            ...
            if(bDebug) std::cout << "[RIGHT] HANGUP:: number of callers" << callers.size() << "\n";
        }
            ...
            if(bDebug) std::cout << "[RIGHT] CALLER(" << callers.size() << "):: " << phoneNumber << "\n";
        }
    }
}
...
void wonderApp::keyPressed(int key){
    ...
    if(bDebug) std::cout << "[RIGHT] mousePressed:: " << callers.size() << "\n";        
}